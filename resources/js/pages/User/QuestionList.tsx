            ...
            <div className="flex flex-col gap-6 p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto w-full">
                <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b pb-4">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        <Button variant={statusFilter === 'all' ? 'default' : 'outline'} onClick={() => setStatusFilter('all')} size="sm" className="h-8 text-xs rounded-full px-4 font-bold">Semua</Button>
                        <Button variant={statusFilter === 'answered' ? 'default' : 'outline'} onClick={() => setStatusFilter('answered')} size="sm" className="h-8 text-xs rounded-full px-4 font-bold bg-primary/10 text-primary border-none hover:bg-primary/20 hover:text-primary">Terjawab</Button>
                        <Button variant={statusFilter === 'unanswered' ? 'default' : 'outline'} onClick={() => setStatusFilter('unanswered')} size="sm" className="h-8 text-xs rounded-full px-4 font-bold">Belum Dijawab</Button>
                        <Button variant={statusFilter === 'submitted' ? 'default' : 'outline'} onClick={() => setStatusFilter('submitted')} size="sm" className="h-8 text-xs rounded-full px-4 font-bold bg-amber-500/10 text-amber-600 border-none hover:bg-amber-500/20 hover:text-amber-700">Diajukan</Button>
                        <Button variant={statusFilter === 'revision' ? 'default' : 'outline'} onClick={() => setStatusFilter('revision')} size="sm" className="h-8 text-xs rounded-full px-4 font-bold bg-destructive/10 text-destructive border-none hover:bg-destructive/20 hover:text-destructive">Perlu Revisi</Button>
                        <Button variant={statusFilter === 'completed' ? 'default' : 'outline'} onClick={() => setStatusFilter('completed')} size="sm" className="h-8 text-xs rounded-full px-4 font-bold bg-green-500/10 text-green-600 border-none hover:bg-green-500/20 hover:text-green-700">Selesai</Button>
                    </div>
                </div>
                ...
                {totalFilteredQs === 0 ? (
                    <div className="py-20 flex flex-col items-center justify-center text-center bg-muted/20 rounded-3xl border border-dashed mt-4">
                        ...
                        <p className="text-xs font-semibold text-muted-foreground/60 mt-1">Tidak ada kuesioner dengan status yang dipilih.</p>
                    </div>
                ) : (
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            <Card className={`h-full border shadow-sm cursor-pointer gap-2 overflow-hidden group-focus-visible/card:ring-1 ring-primary flex flex-col border-t-4 ${statusMap[status].borderTop} ${isAnswered ? 'bg-linear-to-br from-background to-primary/2' : 'bg-background'}`}>
                                                                <CardHeader className="p-3 pb-1.5 space-y-0 text-xs flex-none">
                                                                ...
                                                                <CardContent className="p-3 pt-0 flex flex-col flex-1">
                                                                    <p className="text-xs font-medium leading-snug text-foreground/80 group-hover/card:text-primary transition-colors line-clamp-3 mt-0.5 flex-1">
                                                                        {q.text}
                                                                    </p>
