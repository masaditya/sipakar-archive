...
function AnswerNotes({ answer }: { answer: any }) {
    ...
    return (
        <div className="space-y-6 mt-8 pt-6 border-t-2 border-dashed border-muted/50">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-3">
                    ...
                    <textarea 
                        ...
                        className="w-full rounded-xl border border-input bg-background/50 px-4 py-3 text-sm shadow-sm placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary min-h-[120px] resize-y transition-all"
                        placeholder="Tambahkan catatan, evaluasi, atau feedback untuk jawaban pengguna ini..."
                    />
                </div>
                <div className="space-y-3">
                    <div className="flex items-center gap-2">
                        <span className="text-xs font-black uppercase tracking-widest text-blue-600 block">REKOMENDASI (TIDAK TAMPIL DI USER)</span>
                    </div>
                    <textarea 
                        value={recommendation}
                        onChange={(e) => setRecommendation(e.target.value)}
                        className="w-full rounded-xl border border-blue-200 bg-blue-50/30 px-4 py-3 text-sm shadow-sm placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 min-h-[120px] resize-y transition-all"
                        placeholder="Tambahkan rekomendasi tindak lanjut..."
                    />
                </div>
            </div>
            <div className="flex justify-end pt-2">
                <Button 
                    onClick={handleSave} 
                    disabled={isSaving || (notes === (answer.notes || '') && recommendation === (answer.recommendation || ''))}
...
export default function ReviewAssessment({ pelaksana, aspects }: any) {
    ...
    const [bab2ModalOpen, setBab2ModalOpen] = useState(false);
    const [bab2Form, setBab2Form] = useState({
        type: 'UP',
        up_name: 'Bidang ',
        opd_name: pelaksana.organization?.name || '',
        ttd2_jabatan: `KEPALA ${pelaksana.organization?.name?.toUpperCase() || 'INSTANSI'}`,
    ...
    return (
        <>
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                            ...
                            <div className="flex flex-col sm:flex-row gap-2 mt-1 lg:mt-6 shrink-0">
                                <Button
                                    ...
                                    className="border-blue-500/30 text-blue-600 bg-blue-50/50 hover:bg-blue-100/50 shadow-sm w-full sm:w-auto"
                                >
                                ...
                                <Button
                                    ...
                                    size="sm"
                                    onClick={() => {
                                        setBab2Form(prev => ({ ...prev, type: 'UP', up_name: 'Bidang ' }));
                                        setBab2ModalOpen(true);
                                    }}
                                    className="border-emerald-500/30 text-emerald-600 bg-emerald-50/50 hover:bg-emerald-100/50 shadow-sm w-full sm:w-auto"
                                >
                                    <FileCheck className="w-4 h-4 mr-2" /> Cetak BAB 2
                                </Button>
                                <Button
                                    variant="outline"
                                    size="sm"
            ...
            <Dialog open={rhasModalOpen} onOpenChange={setRhasModalOpen}>
                <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto w-[95vw] rounded-2xl mx-auto border-none p-0">
                    <DialogHeader className="p-6 pb-2 border-b bg-blue-50/50">
                        <DialogTitle className="text-xl font-black text-blue-700">Konfigurasi Risalah Hasil Audit Sementara (RHAS)</DialogTitle>
                    </DialogHeader>
                    <div className="flex flex-col gap-4 p-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-4 bg-muted/20 p-5 rounded-2xl border">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <Button
                                        ...
                                        className="flex-1 font-bold"
                                        onClick={() => setRhasForm({ ...rhasForm, type: 'UK', up_name: 'Sekretariat ' + (pelaksana.organization?.name || '') })}
                                    >RHAS UK</Button>
                                </div>
                            </div>
                    ...
                    <div className="p-6 bg-muted/40 border-t flex flex-col sm:flex-row justify-end gap-3 mt-auto">
                        ...
                        <Button variant="secondary" className="rounded-xl w-full sm:w-auto h-11 px-8 font-bold flex items-center shadow-sm" onClick={() => {
                            ...
                            window.open(`/admin/review/${pelaksana.id}/rhas.pdf?download=1&${params}`, '_blank');
                        ...
                        <Button className="bg-blue-600 hover:bg-blue-700 text-white rounded-xl w-full sm:w-auto h-11 px-8 font-bold shadow-md hover:shadow-lg transition-all" onClick={() => {
                            const params = new URLSearchParams(rhasForm as any).toString();
                            setPreviewModal({ isOpen: true, url: `/admin/review/${pelaksana.id}/rhas.pdf?${params}`, name: `RHAS ${rhasForm.type}: ${pelaksana.organization?.name || pelaksana.name}` });
                            setRhasModalOpen(false);
                        }}>Cetak dan Pratinjau</Button>
                    </div>
                </DialogContent>
            ...
            {/* Modal Input BAB 2 */}
            <Dialog open={bab2ModalOpen} onOpenChange={setBab2ModalOpen}>
                <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto w-[95vw] rounded-2xl mx-auto border-none p-0">
                    <DialogHeader className="p-6 pb-2 border-b bg-emerald-50/50">
                        <DialogTitle className="text-xl font-black text-emerald-700">Konfigurasi Laporan Uraian Hasil Pengawasan (BAB 2)</DialogTitle>
                    </DialogHeader>
                    <div className="flex flex-col gap-4 p-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-4 bg-muted/20 p-5 rounded-2xl border">
                                ...
                                <div className="flex gap-2">
                                    <Button 
                                        type="button"
                                        variant={bab2Form.type === 'UP' ? 'default' : 'outline'} 
                                        className="flex-1 font-bold"
                                        onClick={() => setBab2Form({ ...bab2Form, type: 'UP', up_name: 'Bidang ' })}
                                    >Laporan UP</Button>
                                    <Button 
                                        type="button"
                                        variant={bab2Form.type === 'UK' ? 'default' : 'outline'} 
                                        className="flex-1 font-bold"
                                        onClick={() => setBab2Form({ ...bab2Form, type: 'UK', up_name: 'Sekretariat ' + (pelaksana.organization?.name || '') })}
                                    >Laporan UK</Button>
                                </div>
                            </div>
                    ...
                    <div className="p-6 bg-muted/40 border-t flex flex-col sm:flex-row justify-end gap-3 mt-auto">
                        ...
                        <Button className="bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl w-full sm:w-auto h-11 px-8 font-bold shadow-md hover:shadow-lg transition-all" onClick={() => {
                            const params = new URLSearchParams(bab2Form as any).toString();
                            setPreviewModal({ isOpen: true, url: `/admin/review/${pelaksana.id}/bab2.pdf?${params}`, name: `BAB 2 ${bab2Form.type}: ${pelaksana.organization?.name || pelaksana.name}` });
                            setBab2ModalOpen(false);
                        }}>Cetak dan Pratinjau</Button>
                    </div>
                </DialogContent>
